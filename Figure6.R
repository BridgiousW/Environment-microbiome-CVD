...
forsystolicmeta <- read.csv("RUsysBp.csv",check.names=FALSE)
...
forsystolicmetaSig <- forsystolicmetaSig[forsystolicmetaSig$p_valueR<=0.01,]
...
systol_Sig_microbiome <- read.csv("RuralUrbansystolicmicrob.csv",check.names=FALSE)
...
Y <- VVFF1$setting
summary(Y)
list.keepX <- list(microbiome = c(60, 60), metabolomics = c(161,161))
...
##########panel F###
fordiastolicmeta <- read.csv("RUdiabp.csv",check.names=FALSE)
...
###now we get the microbes
fordiastolic_microbiome <- read.csv("RuralUrbandiastolicmicrob.csv",check.names=FALSE)
...
Y <- VVFF1$setting
summary(Y)
list.keepX <- list(microbiome = c(60,60), metabolomics = c(219,219))
MyResult.diablo <- block.splsda(X, Y,keepX=list.keepX)
plotIndiv(MyResult.diablo)
plotVar(MyResult.diablo)
...
###panel C#####
cp_dias_long<-reshape2::melt(cp_diast)
cp_dias_long_microbes<-cp_dias_long[cp_dias_long$Var1%in%colnames(fordiastolic_microbiomeSig),]
cp_dias_long_microbes<-cp_dias_long_microbes[cp_dias_long_microbes$Var2%in%colnames(fordiastolicmetaSig),]
length(unique(cp_dias_long_microbes$Var1))
length(unique(cp_dias_long_microbes$Var2))
cp_dias_long_microbes$absCorr<-abs(cp_dias_long_microbes$value)
cp_dias_long_microbes_filtered<-cp_dias_long_microbes[cp_dias_long_microbes$absCorr >=0.70,]
...
#####systolic####
cp_sys_long<-reshape2::melt(cp_sys)
cp_cp_sys_long_microbes<-cp_sys_long[cp_sys_long$Var1%in%colnames(systol_Sig_microbiome),]
cp_cp_sys_long_microbes<-cp_cp_sys_long_microbes[cp_cp_sys_long_microbes$Var2%in%colnames(systol_new_vvff_createdSig),]
length(unique(cp_cp_sys_long_microbes$Var1))
length(unique(cp_cp_sys_long_microbes$Var2))
cp_cp_sys_long_microbes$absCorr<-abs(cp_cp_sys_long_microbes$value)
cp_cp_sys_long_microbes_filtered<-cp_cp_sys_long_microbes[cp_cp_sys_long_microbes$absCorr >=0.70,]
# Convert long to wide
...
ggcorrplot(
  ...
  labs(
    title = "Systolic-associated Microbiome–Metabolome Correlation Heatmap",
    x = "Metabolites",
    y = "Microbes",
    fill = "Correlation"
  )
...
forsystolicmetaSig$p_valueR<-round(unlist(as.numeric(forsystolicmetaSig$p.value)),digits = 2)
forsystolicmetaSig <- forsystolicmetaSig[forsystolicmetaSig$p_valueR<=0.01,]
forsystolicmetaSig_filtered <- forsystolicmetaSig %>%
  filter(grepl("^HMDB",HMBD_ID))
forsystolicmetaSig_filtered <- forsystolicmetaSig_filtered%>%mutate(HMBD_ID = paste0("hmdb:", HMBD_ID))

###
metabolites.of.interest <- forsystolicmetaSig_filtered$HMBD_ID
